...
func TestExploreMissingPlanNoPanic(t *testing.T) {
	svc := newProbeService(t)
	defer func() {
		if rec := recover(); rec != nil {
			t.Fatalf("RunExplore panicked: %v", rec)
		}
	}()
	_, err := svc.RunExplore(context.Background(), "pl_missing")
	if err == nil {
		t.Fatal("missing plan must return an error")
	}
}
