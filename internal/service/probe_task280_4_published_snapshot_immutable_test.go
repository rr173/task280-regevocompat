...
func TestPublishedSnapshotIgnoresLaterExplore(t *testing.T) {
	svc := newProbeService(t)
	planID, _, v1, v2 := seedProbePlan(t, svc, "freeze")
	if _, err := svc.VerifyAndExplore(context.Background(), planID); err != nil {
		t.Fatal(err)
	}
	wnd, err := svc.DeclareWindow(planID, v1, v2, model.RuleAdapter,
		`[{"output":"customer_name","inputs":["first_name","last_name"],"op":"concat_space"}]`)
	if err != nil {
		t.Fatal(err)
	}
	...
	snap, err := svc.Publish(context.Background(), planID)
	if err != nil {
		t.Fatal(err)
	}
	frozen := snapshotUnresolved(t, snap.ContentJSON)
	if frozen != 0 {
		t.Fatalf("published snapshot should be compatible, unresolved=%d", frozen)
	}
	if _, err := compat.RevokeWindow(svc.Store(), wnd.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.VerifyAndExplore(context.Background(), planID); err != nil {
		t.Fatal(err)
	...
	if snapshotUnresolved(t, got.ContentJSON) != frozen {
		t.Fatalf("published payload mutated: got %d want %d body=%s", snapshotUnresolved(t, got.ContentJSON), frozen, got.ContentJSON)
	}
}
