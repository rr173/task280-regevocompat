...
func (svc *Service) GetSnapshot(id model.SnapshotID) (*model.CompatSnapshot, error) {
	...
	plan, err := svc.s.GetPlan(snap.PlanID)
	if err != nil {
		return snap, nil
	}
	conflicts, err := svc.s.ListConflictsByPlan(snap.PlanID)
	if err != nil {
		return snap, nil
	}
	...
	snap.ContentJSON = content
	_ = svc.s.SaveSnapshot(snap)
	return snap, nil
}

// ListSnapshots 列出计划的已落库快照。
func (svc *Service) ListSnapshots(planID model.PlanID) ([]*model.CompatSnapshot, error) {
	snaps, err := svc.s.ListSnapshotsByPlan(planID)
	if err != nil {
		return nil, err
	}
	for _, snap := range snaps {
		if refreshed, e := svc.GetSnapshot(snap.ID); e == nil && refreshed != nil {
			snap.ContentJSON = refreshed.ContentJSON
		}
	}
	return snaps, nil
}
