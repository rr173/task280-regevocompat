...
func TestSemanticListDoesNotBlockPlan(t *testing.T) {
	svc := newProbeService(t)
	planID, _, _, _ := seedProbePlan(t, svc, "leak")
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	done := make(chan error, 1)
	go func() {
		if _, err := svc.Store().ListSemantics(); err != nil {
			done <- err
			return
		}
		_, err := svc.GetPlan(planID)
		done <- err
	}()
	select {
	case <-ctx.Done():
		t.Fatal("ListSemantics leaked the DB connection; GetPlan blocked")
	case err := <-done:
		if err != nil {
			t.Fatal(err)
		}
