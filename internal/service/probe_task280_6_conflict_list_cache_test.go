...
func TestConflictListSeesReexplore(t *testing.T) {
	svc := newProbeService(t)
	planID, _, v1, v2 := seedProbePlan(t, svc, "cache")
	if _, err := svc.VerifyAndExplore(context.Background(), planID); err != nil {
		t.Fatal(err)
	}
	first, err := svc.Conflicts(planID)
	if err != nil {
		t.Fatal(err)
	}
	unresolved := 0
	...
	if unresolved == 0 {
		t.Fatal("first explore should have unresolved conflicts")
	}
	if _, err := svc.DeclareWindow(planID, v1, v2, model.RuleAdapter,
	...
	second, err := svc.Conflicts(planID)
	if err != nil {
		t.Fatal(err)
	}
	for _, c := range second {
		if c.Field == "customer_name" && !c.Resolved {
			t.Fatalf("list still shows stale unresolved conflict: %+v", c)
		}
	}
}
